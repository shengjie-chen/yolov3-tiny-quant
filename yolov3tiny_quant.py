...
def yolov3tiny_infer_para_gen(quant_model,ofmch_t,first_chr):
    ...
    for index in convindex:
        ...
        ozp_list.append(ozp)
        azp_list.append(azp)
        weight.tofile('infer_bin_q/'+first_chr+'W'+str(cnt)+'.bin')
        bias.tofile('infer_bin_q/'+first_chr+'B'+str(cnt)+'.bin')
        leakytbl.tofile('infer_bin_q/'+first_chr+'R'+str(cnt)+'.bin')
    ...
    bias.tofile('infer_bin_q/'+first_chr+'B'+str(cnt)+'.bin')
    leakytbl.tofile('infer_bin_q/'+first_chr+'R'+str(cnt)+'.bin')
    sigmoid_tbl.astype(np.float32).tofile('infer_bin_q/'+first_chr+'SG.bin')
    ...
    config_list.astype(np.uint32).tofile('infer_bin_q/'+first_chr+'CG'+'.bin')
